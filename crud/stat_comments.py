...
def count_comment_per_episode():
    ...
    df_count=pd.read_csv('csv_files/exported_comments.csv')
    df_count=df_count[df_count['type'] == 'Episode']
    comments_ep=df_count.groupby(['episode_id']).size()
    comments_ep.to_csv('csv_files/comment_per_episode.csv')
...
def count_letters_comments_per_episode():
    ...
    df_count.to_csv('csv_files/numberLetter_comment_per_episode.csv')
